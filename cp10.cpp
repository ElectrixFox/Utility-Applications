...
Functions:      main MainMenu GenerateDJNumber DoesDJExist GetDJNumberInput ViewDJAccess DJs DJMenu AddDJ RemoveDJ ViewDJ
				ViewDJMenu ViewDJByName ViewByDJNumber ViewDJByMusicStyle ChangeDJ ChangeDJMenu ChangeDJName ChangeDJPerformanceName ChangeDJPhoneNumber ChangeDJEmailAddress
				ChangeDJMusicStyle ReadBackGuestFile ReWriteGuestFile GenerateGuestNumber Guests GuestsMenu ViewGuest ViewGuestMenu ViewGuestByName ViewGuestByGuestNumber
				ViewGuestNextAppearance ViewGuestByNextAppearance ViewGuestByGenre AddGuest ChangeGuest ChangeGuestMenu ChangeGuestName ChangeGuestContactName ChangeGuestContactPhoneNumber ChangeGuestContactEmailAddress
				ChangeGuestGenre RemoveGuest ReadBackMusicFile ReWriteMusicFile MusicLongestLength GenerateCatalogueNumber ConvertPlaytimeToSecconds Music MusicMenu Catalogue
				CatalogueMenu ViewCatalogue ViewCatalogueMenu ViewCatalogueByCatalogueNumber ViewCatalogueByTitle ViewCatalogueByArtist ViewCatalogueByGenre ViewCatalogueByPopularity CatalogueAddMusic CatalogueChangeMusic
				CatalogueChangeMusicMenu ChangeMusicTitle ChangeMusicArtist ChangeMusicPlaytime ChangeMusicGenre ChangeMusicCheck CatalogueRemoveMusic ReadBackPlaylistFile ReWritePlaylistFile CalculatePlaylistDuration
				PlaylistDurationCheck FindSongInPlaylist Playlists PlaylistsMenu ViewPlaylist CreatePlaylist EditPlaylist EditPlaylistMenu AddMusic RemoveMusic
				RemovePlaylist ReadBackUserFile ReWriteUserFile GetUserDJ GetGLOA Login Admin AdminMenu AddUser ChangeUser
				ChangeUserMenu ChangeUserUsername ChangeUserPassword ChangeUserAccessLevel RemoveUser Backup Recovery ReadBackEventLinksFile ReWriteEventLinksFile EventsLongestLength
				GuestEventLongestLength ReadBackEventFile ReWriteEventFile GenerateEventNumber GetAttendingDJ GetAttendingDJs IsDJAttending IsRecurringEvent Events EventsMenu
				ViewEvents ViewEventsMenu ViewEventsByEventReference ViewEventsByDate ViewEventsByTitle AddEvent AddEventMenu DJSession GuestAppearance ExternalEvent
				Replay ChangeEvents ChangeEventsMenu ChangeEventsTitle ChangeEventsDate ChangeEventsTime ChangeEventsLocation ChangeEventsDescription ChangeEventsCost ChangeEventsContact
				ChangeEventsPlaylist ChangeEventsContact ChangeEventsContactMenu ChangeEventsContactName ChangeEventsContactEmailAddress ChangeEventsContactPhoneNumber AllocateDJs RemoveEvents ReadBackScheduleFile ReWriteScheduleFile
				GetScheduleYear IsEventOnSchedule FindScheduleGuestSlot CalculateScheduleDateIndex CalculateScheduleTimeIndex GetScheduleGuestNumber GetScheduleGuestNumber GetScheduleEventNumber GetEventDateOnSchedule GetDayInMonth
				IsSlotUsed FindAllEventsOfTypeBetweenDates FindAllEventsOfTypeBetweenDates FindAllEventsBetweenDates FindAllEventsBetweenDates FindAllEventsBetweenTimes FindAllHolidaySlots FindAllHolidaySlots Scheduling SchedulingMenu
				Schedule ScheduleMenu ViewSchedule ViewScheduleMenu ViewScheduleByDJ ViewScheduleAvailabilityByDJ ViewScheduleGuestSlots CreateSchedule EditSchedule EditScheduleMenu
				ChangeDJAvailability GuestSlots IsHolidayApproved Holidays HolidaysMenu ViewHolidays ViewHolidaysMenu ViewHolidaysByDJ ViewHolidaysByDate ViewHolidaysByUnapproved
				BookHolidays ChangeHolidays ChangeHolidaysMenu ChangeHolidaysStartDate ChangeHolidaysEndDate RemoveHolidays ApproveHolidays FindInArray64 FindInArray48 FindInArray32
				FindInArray24 FindInArray10 FindInArray04 FindInArray03 FindInArrayInt FindPartialInArray32 FindInString FindCharInString FindLongestInIndecies FindLongest
				MakeArrayUniqueInt FindNextReferenceNumber FindDayOnDate GetHeaders CalculateNumericalDate CalculateNumericalDateInt ConvertMonth CalculateDuration DateComp TimeComp
				GetSystemsClock AddTimes CompareWordsAlphabetically BinaryDateSearch GetMenuChoice DateRangeCk IsNumber RangeCk PresenceCk LengthCk
				DateValidation ValidateEmailAddr PhoneNumberFormatCheck ValidGenreCk YesNoValidation MultiOptionValidation ValidatePrice EventCollisionCheckTime EventCollisionCheckEvent IsDJAvailable
				IsGuestSlotAvailable PasswordSecurityCheck ValidateFilePath
Description:    
Author:         Luke Phillips
Environment:    Borland C++ Pro 6.0
Notes:			
Revisions:      23/02/2024
******************************************************************************/
...
// finds an item in the array 3 bytes with default starting point at 0
int FindInArray03(char array[][3], char item[], int size, int start = 0);
...
void InputGuestNumberCk(char inguno[10], char outmsg[], int quitoptn = 0);
...
// if the user number is -1 then checks if the password is new
void InputPasswordCk(char inpass[64], char outmsg[], int userno = -1);
...
confirm = InputYNChoiceCk("\nWould you like to confirm this change (y/n): ");
if(confirm == 'y')
	{
...
if(ngi == 0)
	...
	getch();
	vistat = 6;	// stop the system going into the loop
	} //endif guest file is empty
...
if(ngi == 0)
	...
	cout << "\nError: No Guests Found" << "\nPress any key to return";
	getch();
	chstat = 6;	// stop the system going into the loop
	} //endif guest file is empty
...
if(nmi == 0)
	...
	getch();
	viewstatus = 6;	// stop the system going into the loop
	} //endif catalogue is empty
...
if(fpos != -1)
	...
	choice[0] = InputYNChoiceCk("\nEnter Check (Y/N): ");	// gets the choice input for the check
	choice[1] = '\0';
...
if (adch == 'y')
	{
	AddPlaylistEntry(inevref, incatno);	// adds the song to the playlist
	PlaylistDurationCheck(inevref);	// checking the playlists duration
	} //endif the song should be added
...
if(nui == 0)
	...
	cout << "\nUsername: admin";
	cout << "\nPassword: root";
	InputUsernameCk(inusrnam, "\nEnter new username: ", 1);	// gets new username
	InputPasswordCk(inpasswrd, "\nEnter new password: ");	// gets new password

	strcpy(usernames[nui], inusrnam);	// sets the new admin username as inusernam
	strcpy(passwords[nui], inpasswrd);	// sets the new admin password as inpasswrd
	strcpy(usrnos[nui], "0");	// the null DJ is DJ 0
	strcpy(loas[nui], "3");	// level of access 3

	nui = 1;
	itoa(nui, nuc, 10);	// converts the number of users to a string
	ReWriteUserFile();

	curusr = 0;	// setting as admin for first time login
	}//endif the first time logging in
else
	{	
	while(cont == 0)
		{
		NameInputPresCk(inusrnam, "\nEnter username: ");	// gets the inputted username
		...
		if(strcmpi(inpasswrd, "X") != 0)
			{
			cont = 1;	// stops the loop
			} //endif the password doesn't match the user
		} //endwhile loop until the correct username and password are entered
	
	curusr = atoi(usrnos[usrloc]);	// getting the number of the user
	} //endelse it isn't first time logging in
...
while(cont == 0)
	{
	InputPasswordCk(passwrd, "\nEnter New Password: ");	// asks for new password
	InputPasswordCk(passwrd2, "\nEnter New Password Again: ");	// asks for the 2nd entry of the password
	
	if(strcmp(passwrd, passwrd2) == 0)
		{
		cont = 1;
		} //endif the passwords are the same
	else
		{
		cout << "\nError: Passwords don't match";
		} //endelse the passwords aren't the same
...
int loc;
int count;
int uloc;	// location of user to search for
...
if(atoi(loas[loc]) == 3)	// checks if the inputted user is level 3
	{
	uloc = FindInArray03(loas, "3", nui);	// finds the first level 3 user
	if(uloc == loc)
		{
		uloc = FindInArray03(loas, "3", nui, loc + 1);
		} //endif the first level 3 user isn't the inputted user
	if(uloc == -1)
		{
		cout << "\nError: Cannot delete the only level 3 user";
		choice = 'n';
		} //endif the first level 3 user before the admin is the one to delete and there aren't any level 3 users after the admin then cancel the deletion
	} //endif the access level is 3
...
if(choice == 'y')
	{
	cout << "\nPlease create a folder called 'Backups' for all of the backups to be stored";
	InputFilePathCk(inpfp, "\nEnter Backup File Path: ");
	inplen = strlen(inpfp);

	if(inpfp[inplen - 1] != '\\')
		{
		inpfp[inplen - 1] = '\\';	// sets it as a backslash
		} //endif the final character isn't a backslash
...
else if(strcmpi(evlocation[loc], "Station Repeat") == 0)
	{
	type = 'r';
...
itoa(evno, evnoc, 10);	// converts the number to a string
OutputEvent(evnoc);
}
...
for(count = 0; count < nei; count++)
	...
	if(type == 'r')
		...
		strcpy(evlocation[count], "Station");	// hides their location tag
...
if(nei == 0)
	...
	cout << "\nError: No Events Found" << "\nPress any key to return";
	getch();
	viewstatus = 4;	// stop the system going into the loop
	} //endif there are no events
...
InputDateCk(indate, "\nEnter Date: ");	// gets validated input date
itoa(idjno, indjno, 10);	// gets the guest number
...
GenerateEventNumber(evno);	// generates the event number
...
if(nei == 0)
	...
	cout << "\nError: No Events Found" << "\nPress any key to return";
	getch();
	chstat = 9;	// stop the system going into the loop
	} //endif events file is empty

while (chstat != 9)
	{
	chstat = ChangeEventsMenu();
	clrscr();
	} //endwhile
...
GetAttendingDJs(inevno, djnos, 1);	// outputs the attending DJs
...
if(top == 0)
	{
	cout << "\nNo holidays found";
	} //endif there are no holidays found
else
	{
	OutputHolidays(holrefs, top, 1);
	} //endelse there are holidays found
...
if(nei != 0)
	...
	} //endif there are events sort them
...
GetToday(today);
InputDJNumberCk(djnoc, "\nEnter DJ Number: ");  // gets the valid DJ number for an existing DJ
InputDateCompCk(sdate, "\nEnter Start Date: ", today); // gets validated start date after today

idjno = atoi(djnoc);    // gets the DJ number as an integer

while(cont == 0)
    {
	InputDateCompCk(edate, "\nEnter End Date: ", sdate);	// gets validated date after the start date
...
while(cont == 0)
	...
	if(DateComp(ndate, today) != 1)
		{
		cout << "\nError: Inputted date not after today";
		continue;	// restarts the loop
		} //endif the date isn't after the current date
...
for (count = start; count < size; count++)
        {
        compare = strcmp(array[count], item);
        if(compare == 0)
                {
                return count;
                } //endif
...
int FindInArray03(char array[][3], char item[], int size, int start)
{
int count = 0;
int compare = 0;
...
if ((output == 1) && (isvalid != 0))
	...
	if(isvalid == -1)
		{
		cout << "\nError: Inputted Value Below Acceptable Range";
		} //endif isvalid is below the acceptable range
	else
		{
		cout << "\nError: Inputted Value Above Acceptable Range";
...
int valid = 1;
int loc;
char genres[][24] = { "Rock", "Pop", "Electronic", "Jazz", "Classical", "Heavy rock", "Metal", "Hip Hop", "Rap", "Country", "Electric", "Dance", "Soul", "Folk", "Musical Theatre", "House", "Disco", "Techno", "Alternative" };

loc = FindInArray24(genres, inpgenre, 19);
...
while(cont == 0)
    {
    cout << outmsg;
	cin.getline(choice, 10);  // getting the input choice
	cont = 1;	// assuming it is true

	if(LengthCk(choice, 1, 2) != 1)
		{
		cont = 0;
		continue;	// restarts the loop		
		}
	else if(YesNoValidation(choice[0]) != 1)
		{
		cont = 0;
		continue;	// restarts the loop
		} //endelseif there is a valid yes/no
    } //endwhile loop until a valid option has been entered

return choice[0]; // copying the choice into the choice parameter
}
...
while(cont == 0)
	...
	if(strlen(tmpdate) == 1)
		{
		count = FindCharInString(&additionals[dotck], tmpdate[0], strlen(additionals) - dotck);	// starting at dotck as if there is a dot to start the first char isn't wanted
	...
	if(DateValidation(tmpdate) == 1)
		{
		comp = DateComp(tmpdate, today);

		if(comp != -1)
			{
			cout << "\nError: Inputted date before today";
			continue;	// restarts the loop
			} //endif the date is in the past
		} //endif the date is invalid then 0 - 1 will be -1 and if it is valid it will be 0

	cont = 1;	// ends the loop
...
while(valid == 0)
	...
	comp = DateComp(compdate, tmpindat);	// doing the comparison
...
void InputGuestNumberCk(char inguno[10], char outmsg[], int quitoptn)
{
...
while (cont == 0)
	...
	if(userno == -1)
		...
		} //endif the password has to be new
	else if((userno >= 0) && ((find == -1) || (atoi(usrnos[find]) != userno)))
		{
		cout << "\nError: Invalid Login";
		strcpy(tmppass, "X");	// marks it as wrong
		break;	// stops looping
		} //endelseif the password should match a given user and the found user isn't the required user
	else
		{
		cont = 1;
		} //endelse is valid so cont = 1
