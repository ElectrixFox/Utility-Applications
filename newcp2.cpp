...
if(idjno < 1 || idjno > 52)
	{
	return 0;
	} //endif the number is out of range
...
OutputSongs(songs, "TiPlArGePuIdCh", top);
...
if(output == 0)
	return curdur;
// return if the details shouldn't be outputted
...
while (muloc == -1)
	...
	// finds the song
	muloc = FindInArray10(mucatno, incatno, nmi);
	} //endwhile a song hasn't been found request the input again
...
while(choice == 'y')
	...
	if(muloc == -1)
		continue;
	// if the song doesn't exist it repeats the request
...
// iterating through all DJs and adding this to their schedule
for(djit = 1; djit < 52; djit++)
	{
	// check if the DJ is attending any events at that time
	// in future change this to a procedure as it will need to be reused
	idate = CalculateScheduleDateIndex(indate);
	itime = CalculateScheduleTimeIndex(intime);
	
	GetScheduleSlot(slot, idate, itime, djit);
	
	if(slot[0] == '*')
		{
		AddEventToSchedule(evnoc, 'R', djit);
		} //endif the slot is empty add the event to the DJs schedule

	} //endfor loop through the DJs
...
/* for (count = 0; count < back2; count++)
	{
	AddPlaylistEntry(evnoc1, catnos2[count]);
	} //endfor

for (count = 0; count < back1; count++)
	{
	AddPlaylistEntry(evnoc2, catnos1[count]);
	} //endfor */
...
if(GetScheduleType(date, time, djid) == 'H')
	{
...
// To-Do: change this to a separate routine
...
for(count = 0; (count < nli); count++)
	{
	loc = FindInArray10(evlinkevno, inevno, nli, loc + 1);

	if(atoi(evlinkdjno[loc]) == indjno)
		{
		return;
		} //endif they are the same return early as they don't need adding
	if(loc == -1)
		{
		break;
		} //endif the event hasn't been found
	} //endfor loop through all attending DJs to check that the new DJ isn't a duplicate
...
for(count = 0; count < duri; count++)
	...
	// sets the schedule slot
	SetScheduleSlot("*", idate, curhour, indjno);
	} //endfor loop though duration of event
}
...
// copying the start date into the end date for the addition to find the end date
strcpy(edate, evdate[evloc]);
